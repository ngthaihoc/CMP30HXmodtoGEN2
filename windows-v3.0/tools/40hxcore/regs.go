...
// LinkTargetAllowed: checks whether endpoint and root support target link speed under profile ceiling
func LinkTargetAllowed(endpointMax, rootMax, profileCeiling, target uint32) bool {
	if target == 0 || target > profileCeiling {
		return false
	}
	if endpointMax < target || rootMax < target {
		return false
	}
	return true
}

// PcieMaxSpeed: read Max Link Speed (bits [3:0]) from LNKCAP
func PcieMaxSpeed(wh syscall.Handle, bdf uint32) uint32 {
	cap := PcieCap(wh, bdf)
	if cap == 0 {
		return 0
	}
	v, err := PciRd(wh, bdf, cap+0x0C)
	if err != nil {
		return 0
	}
	return v & 0xF

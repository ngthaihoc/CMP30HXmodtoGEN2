...
//
// 部署模型 (v2.5 起):
//   1. 持久备份源  %ProgramData%\40HXUnlock\drivers\*.sys
...
type DrvDeploy struct {
	...
	BackupOK   bool     // %ProgramData%\40HXUnlock\drivers 备份源存在且 >0
...
// PdDrvDir: %ProgramData%\40HXUnlock\drivers (安装器写入的持久备份源)
func PdDrvDir() string {
	base := os.Getenv("ProgramData")
	if base == "" {
		base = `C:\ProgramData`
	}
	return filepath.Join(base, "40HXUnlock", "drivers")
}

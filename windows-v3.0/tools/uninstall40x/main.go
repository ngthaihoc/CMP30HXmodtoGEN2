...
func main() {
	...
	fmt.Println("==============================================")
	fmt.Println("  Công cụ gỡ cài đặt CMP 40HX Windows Unlock v3.0.0")
	fmt.Println("  Gỡ bỏ: Tác vụ lịch trình / Mục khởi động mở khoá / ESP EFI / Tự khởi động Gen2 / Driver")
	fmt.Println("==============================================")
	if !isAdmin() {
		if hasArg("-elevated") {
			// 已提权过一次仍失败(静默提权策略/受限token) -> 禁止再循环, 直接报错
			msgbox("Công cụ gỡ cài đặt 40HX", "Yêu cầu quyền thất bại: Tài khoản hiện tại không thể nhận quyền quản trị.\nVui lòng nhấp chuột phải vào tệp -> Chọn Run as administrator.", mbIconError)
			return
		}
		selfElevate()
		return
	...
	if lockOnce(`Local\40HXUninstaller_v1`) == nil {
		msgbox("Công cụ gỡ cài đặt 40HX", "Trình gỡ cài đặt đang chạy, vui lòng không nhấp trùng lặp.", mbIconInfo)
		return
	}
	...
	// 2. Gen2 Run 键
	fmt.Print("[2/8] Xoá tự khởi động Gen2 khi đăng nhập ... ")
	delRunKey()
	fmt.Println("Hoàn thành")
	...
	// 6. 驱动文件
	fmt.Println("[6/8] Xoá tệp driver...")
	...
	fmt.Print("[6.6/8] Dọn dẹp %ProgramData%\\40HXUnlock + Khoá chính sách ... ")
	delProgramData()
	fmt.Println("Hoàn thành")

	// 6.7 Defender 排除清理(安装时加的白名单, 卸载需移除不留残留)
	fmt.Print("[6.7/8] Dọn dẹp danh sách loại trừ Defender ... ")
	...
	fmt.Println("  Lưu ý: Thiết lập nguồn khi cài đặt (Fast Startup/tiết kiệm điện PCIe) được giữ nguyên, cách khôi phục xem trong README.")
	if silent {
		return
	}
	icon := uint(mbIconInfo)
	txt := "Gỡ cài đặt hoàn tất.\nKhuyến nghị khởi động lại máy tính.\n" +
		"\nLưu ý: Thiết lập nguồn khi cài đặt (Fast Startup/tiết kiệm điện PCIe)\nđược giữ nguyên theo sở thích nguồn, cách khôi phục xem README §2.4.\n"
	if leftover != "" {
		icon = mbIconError
		txt += "\nVẫn còn tàn dư:\n" + leftover
	}
	txt += "\nNhật ký chi tiết: " + filepath.Join(os.TempDir(), "40HX_uninstaller.log")
	msgbox("Công cụ gỡ cài đặt 40HX", txt, icon)
}

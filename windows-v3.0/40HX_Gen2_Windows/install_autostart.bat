@echo off
rem ============================================================
rem  40HX Gen2 BYOVD unlock - register LOGON autostart (admin)
...
echo [2/3] register scheduled task "40HXGen2" (logon, highest)...
set "SCRIPT_CMD=cmd /c cd /d "%~dp0" && "%PYEXE%" 40hx_gen2_byovd.py --selfclean"
schtasks /Create /F /TN "40HXGen2" /TR "%SCRIPT_CMD%" /SC ONLOGON /RL HIGHEST /RU "%USERNAME%" 2>&1
if errorlevel 1 (echo   task create returned error) else (echo   ok)

echo [3/3] unlock once now to verify...
"%PYEXE%" 40hx_gen2_byovd.py --selfclean
echo.
echo Done.  remove autostart:  schtasks /Delete /TN "40HXGen2" /F
pause
exit /b 0
:err
